...
std::string My::Susurrador::manageAudio(const QBuffer& buffer) {
    std::string output;

    if (whisper_full(context_m, whisperParams_m, reinterpret_cast<const float*>(buffer.buffer().constData()), buffer.size() / sizeof(float)) == 0) {
